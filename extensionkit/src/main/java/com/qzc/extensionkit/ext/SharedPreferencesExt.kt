...
/**
 * created by qzc at 2019/09/22 12:23
 * desc:
 */
...
fun <T> Context.putValue(key: String, value: T, name: String = packageName) = sp(name).edit {
    when (value) {
...
fun <T> Activity.putValue(key: String, value: T, name: String = packageName) = sp(name).edit {
    when (value) {
        is Long -> putLong(key, value)
        is String -> putString(key, value)
        is Int -> putInt(key, value)
...
fun <T> Context.getValue(key: String, default: T, name: String = packageName): T = sp(name).run {
...
fun <T> Activity.getValue(key: String, default: T, name: String = packageName): T = sp(name).run {
    val result = when (default) {
        is Long -> getLong(key, default)
        is String -> getString(key, default)
        is Int -> getInt(key, default)
